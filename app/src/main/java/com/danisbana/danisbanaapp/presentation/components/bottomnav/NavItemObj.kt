...
class NavItemObj(
    ...
    companion object {
        ...
        fun adminGenerate(): Array<NavItemObj> {
            return arrayOf(
                NavItemObj(
                    icon = Icons.Default.Email,
                    route = Screen.ConditionalScreen
...
@Composable
fun getBottomNavigationItems(isAdmin: Boolean = false) = remember(isAdmin) {
    if(isAdmin) NavItemObj.adminGenerate()
    else NavItemObj.generate()
}
